...
diferenca1Emprego <- (data.frame(PIB[2:18],pdemprego))                   #Exibe a tabela da série original coma diferença <- 
DIFERENCAEmprego <- ts(diferenca1Emprego, start = 1994, frequency = 1)   #Define serie temporal para a tabela diferenca1
plot(DIFERENCAEmprego, plot.type="single", col=c("Black","Green"))   #Cria o grafico com as duas series
plot(pdeEmprego, type="l")             
...
TesteADF_pdEmprego_trend <- ur.df(pdemprego, "trend", lags = 1)         #Teste DF-DickFuller com drift e com tendencia
summary(TesteADF_pdEmprego_trend)

TesteADF_pdEmprego_drift <- ur.df(pdemprego, "drift", lags = 1)         #Teste DF-DickFuller com drift e com tendencia
summary(TesteADF_pdEmprego_drift)

TesteADF_pdEmprego_none <- ur.df(pdemprego, "none", lags = 1)         #Teste DF-DickFuller com drift e com tendencia
summary(TesteADF_pdEmprego_none)
...
arima113 <- arima(emprego, c(1,1,3))
...
arima110 <- arima(emprego, c(1,1,0))
arima111 <- arima(emprego, c(1,1,1))
arima112 <- arima(emprego, c(1,1,2))
...
arima210 <-  arima(emprego, c(2,1,0))
arima211 <-  arima(emprego, c(2,1,1))
arima212 <-  arima(emprego, c(2,1,2))
arima213 <-  arima(emprego, c(2,1,3))
...
arima011 <-  arima(emprego, c(0,1,1))
arima012 <-  arima(emprego, c(0,1,2))
arima013 <-  arima(emprego, c(0,1,3))
#AR
arima020 <-  arima(pib, c(0,1,0))

arima020 <-  arima(emprego, c(0,1,0))
